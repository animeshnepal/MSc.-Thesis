...
\ce{H2O, TIC} and calcium) and the relationship with each another. A two-dimensional model domain [5mm $\times$ 15mm] 
...
parameter (\cref{chapter:results}) -- were set up in \DuMuX as illustrated in \cref{tab:scenarios}. 
\begin{enumerate}
...
Flowchart in \cref{fig:flowchartReactionSource} shows the reaction source function's implementation and flowcharts in appendix \ref{app:appendixA} 
...
\newpage
\centering
\caption{Flowchart for reactionSource function in \DuMuX}
\begin{tikzpicture}[node distance=2cm]
