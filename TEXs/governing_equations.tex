...
The chapter deals with the terms' definition and governing equations used in the model that was 
implemented in the numerical simulator \DuMuX and \MATLAB. It also explains the mathematical model 
in implementing reactive source and sink terms. The conservation of mass and momentum equations, 
and the governing chemical kinetics in the implemented numerical method, together with the boundary 
conditions and initial conditions that are necessary to solve the differential equations are explained. 
...
is limited modeling of calcite dissolution in the cave surfaces. The conservation of a 
...
\end{itemize}
The density of water increases slightly with the dissolution of \ce{CO2}\cite{garcia2001density} and calcium-carbonate 
\cite{zhao2015solubility}. Although the pressure we are interested in is fairly low (1 atm) compared to the pressures 
in \cref{fig:DensityH2OCO2}, we can imply that the \ce{H2O-CO2} density will increase as more \ce{CO2} is added into the karst water. \\
For the modeling purpose, we employ a limitation in the model as we did not consider the density change due to the addition 
of \ce{CO2} and calcium into the aqueous solution, making it less computationally expensive. We are interested in developing a 
2D model of size 5mm $\times$ 15mm that implements calcite dissolution as a source/sink term, so the employed limitations will 
have negligible influence, if any, in altering the predicted time scale for the rate of dissolution. \\
...
the simulation time. Based on the nature of the boundary value, boundary conditions are categorized into two types: 
\begin{itemize}
...
\section{Reactive source and sink terms}\label{sec:reactivesource} The source and sink terms account for the dissolution of 
calcite and consumption of CO\textsubscript{2}. There could be several other minerals found in caves, but our scope of 
the study is limited to calcite. \\
For the purpose of our study, we assumed abundant calcite in our model -- and for an open system (see \cref{chapter:modelconcept}) 
...
Calcite dissolution occurs until the liquid phase saturates or all of the calcite is consumed.
...
Dissolution of calcite produces \ce{Ca^{2+}}, \ce{CO_3^{2-}}, \ce{HCO_3^{-}}, \ce{H^+}, and \ce{OH^-} ions in the solution. 
...
molality of hydrogen ion, see figure \ref{fig:dissKinetics}. At the beginning, when there is no calcium in the solution, 
rate of dissolution is the highest. As the time passes, calcium concentration in the solution increases, thereby decreasing 
the rate of dissolution, and finally system reaches steady-state. Time to reach equilibrium depends on several factors such as 
velocity of fingers, initial pH of the solution, initial TIC and amount of \ce{CO2} at the top of the domain, which will be 
...
respectively. The charge balance states that:
\begin{equation}
...
constant, and consequently $m^\kappa$ also has less fluctuations. So, we used molality while solving charge balance equation 
