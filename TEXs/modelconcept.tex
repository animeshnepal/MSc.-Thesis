...
the same model size as in \DuMuX, which would make the dissolved products to instantly mix throughout the domain as the 
dissolution proceeds. In \MATLAB we solved for a closed system with no outflow or inflow of calcium, \ce{CO2} and water. 
...
\paragraph*{\DuMuX} In \DuMuX, continuity equation \ref{eq:contiEq} was solved together with source and sink terms accounting for 
\ce{CO2} and calcium. A few batch scenarios with varying parameters, such as Initial pH, reactive-area/batch volume (if it's an closed system), 
flow-velocity (if it's an open system), \ce{CO2} concentration at the cave above the karst water (if it's an open system) and grid grading 
parameter (\ref{chapter:results}) were setup in \DuMuX as illustrated in \cref{tab:scenarios}. 
\begin{enumerate}
    \item Closed System: In order to make a plausible comparison with the \MATLAB results, the top and bottom boundary 
    ...
    and  protruding \ce{CO2} fingers into the karst water were set to model cave scenario. Initial pH, flow-velocity, \ce{CO2} 
    concentration at the cave above the karst water and grid grading parameter were varied to understand its dependency on the 
...
to calculate rate of calcite dissolution (\ce{r_{diss}}). Rate of calcite dissolution when multiplied with the reactive-area 
and the time-step would give the amount of produced \ce{CO2} and calcium. With the increase in calcium concentration in the 
...
The fluid-system had one phase (liquid phase) and three components (\ce{H2O, CO2, calcium}). We only considered 
...
\subsection*{Primary variables} Pressure and concentration  of \ce{CO2}, calcium and \ce{H2O}, three scalar variables, in 
terms of mole fraction as well as the velocity vector were selected as primary unknowns. 

\subsection*{Boundary conditions} 
The velocity vector ($\mathbf{v}$), pressure (p) and the concentrations of \ce{CO2} and calcium are set to Dirichlet values; 
and \ce{H2O}, \ce{CO2} and calcium fluxes are set to Neumann values in the NS-model. We had two setups in \DuMuX i.e., open 
and closed systems, and boundary conditions were different for these two setups.

\paragraph*{Open system} \mbox{} \\

As shown in \cref{fig:OpenSystem}, we set Dirichlet condition at the top boundary for velocities in x and y directions and 
concentration of \ce{CO2} and calcium. For the left boundary, we assumed no-slip condition as we set Dirichlet condition for 
velocities and Neumann condition for \ce{CO2} and calcium fluxes. Similarly for the bottom boundary, we set Dirichlet condition 
for pressure and Outflow conditions for \ce{CO2} and calcium fluxes formed due to the dissolution of calcite. we set a symmetric 
...
Neumann no-flow for concentration of \ce{CO2} and calcium fluxes. For the left boundary, we assumed no-slip condition -- similar 
to the open system -- as we set Dirichlet condition for velocities and Neumann condition for \ce{CO2} and calcium fluxes. On the 
bottom boundary, we set Dirichlet condition for pressure and Neumann no-flow for concentration of \ce{CO2} and calcium fluxes. 
We also set a symmetric property at the right boundary -- again, similar to the open system. Hence, the only difference between 
...
\ce{CO2} and calcium into the solution because of dissolution of calcite. On the one hand addition of \ce{CO2} in karst water in 
an open system  will form carbonic acid which will reduce the pH, on the other hand addition of carbonic acid replenishes dissolution 
potential of the karst water, thereby enhancing dissolution of calcite forming carbonates and bicarbonates which will increase the pH. 
Therefore, steady-state is expected to be dependent on the flow-velocity with which \ce{CO2} is being carried into the karst water from 
the atmosphere adjusting to new acidity and a consequence alkalinity.
...
The saved values was then retrieved and used for visualization in paraview. The function \code{reactionSource()} needs molality of 
hydrogen ($\mathrm{m^{H^+}}$), which is already calculated and stored in a vector, to calculate the added concentration of \ce{CO2} 
...
\node (pro3) [process, below of=pro2, yshift=-0.5cm] {calculate Calculate $\mathrm{m^{CO_3^{2-}}}$ = $\frac{\ce{m^{TIC}}}
